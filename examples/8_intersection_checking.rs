...
use optima::scenes::robot_geometric_shape_scene::{EnvObjPoseConstraint, EnvObjShapeRepresentation, EnvObjSpawner, RobotGeometricShapeScene, RobotGeometricShapeSceneQuery};
...
fn main() {
    ...
    let env_obj_idx = robot_geometric_shape_scene.add_environment_object(EnvObjSpawner::new("sphere", Some(0.1), Some(EnvObjShapeRepresentation::BestFitConvexShape), None, None), false).expect("error");
    robot_geometric_shape_scene.update_env_obj_pose_constraint(env_obj_idx, EnvObjPoseConstraint::Absolute(OptimaSE3Pose::new_from_euler_angles(0.,0.,0.,0.5,0.5,0., &OptimaSE3PoseType::ImplicitDualQuaternion))).expect("error");
