    ...
    pub fn displacement(&self, other: &OptimaSE3Pose, conversion_if_necessary: bool) -> Result<OptimaSE3Pose, OptimaError> {
        ...
        if !c {
            return if conversion_if_necessary {
                ...
                Err(OptimaError::new_string_descriptor_error("incompatible pose types in displacement."))
            }
        }
        ...
        return match self {
            OptimaSE3Pose::ImplicitDualQuaternion { data, .. } => {
                ...
                match other {
                    ...
                    _ => { Err(OptimaError::new_string_descriptor_error("incompatible pose types in displacement.")) }
                }
            }
            OptimaSE3Pose::HomogeneousMatrix { data, .. } => {
                let data0 = data;
    ...
    pub fn distance_function(&self, other: &OptimaSE3Pose, conversion_if_necessary: bool) -> Result<f64, OptimaError> {
        ...
        if !c {
            return if conversion_if_necessary {
                ...
                Err(OptimaError::new_string_descriptor_error("incompatible pose types in distance function."))
            }
        }
        ...
        return match self {
            OptimaSE3Pose::ImplicitDualQuaternion { data, .. } => {
                ...
                match other {
                    ...
                    _ => { Err(OptimaError::new_string_descriptor_error("incompatible pose types in distance function.")) }
                }
            }
            OptimaSE3Pose::HomogeneousMatrix { data, .. } => {
                let data0 = data;
