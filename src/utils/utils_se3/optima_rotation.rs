...
/// rotation types and functions over singular or pairs of rotations.
#[derive(Clone, Debug, Serialize, Deserialize)]
pub enum OptimaRotationAD {
    RotationMatrix{data: Rotation3<f64ad>, rotation_type: OptimaRotationType },
    UnitQuaternion{data: UnitQuaternion<f64ad>, rotation_type: OptimaRotationType }
}
impl OptimaRotationAD {
    pub fn new_rotation_matrix(data: Rotation3<f64ad>) -> OptimaRotationAD {
        OptimaRotationAD::RotationMatrix { data, rotation_type: OptimaRotationType::RotationMatrix }
    }
    pub fn new_unit_quaternion(data: UnitQuaternion<f64ad>) -> OptimaRotationAD {
        OptimaRotationAD::UnitQuaternion { data, rotation_type: OptimaRotationType::UnitQuaternion }
    }
    pub fn new_unit_quaternion_identity() -> OptimaRotationAD {
        OptimaRotationAD::UnitQuaternion { data: Default::default(), rotation_type: OptimaRotationType::UnitQuaternion }
    }
    pub fn new_rotation_matrix_identity() -> OptimaRotationAD {
        OptimaRotationAD::RotationMatrix { data: Default::default(), rotation_type: OptimaRotationType::RotationMatrix }
    }
    pub fn new_rotation_matrix_from_euler_angles(rx: f64ad, ry: f64ad, rz: f64ad) -> OptimaRotationAD {
        let data = Rotation3::from_euler_angles(rx, ry, rz);
        return Self::new_rotation_matrix(data);
    }
    pub fn new_unit_quaternion_from_euler_angles(rx: f64ad, ry: f64ad, rz: f64ad) -> OptimaRotationAD {
        let q = UnitQuaternion::from_euler_angles(rx, ry, rz);
        return Self::new_unit_quaternion(q);
    }
    pub fn new_rotation_matrix_from_axis_angle(axis: &Unit<Vector3<f64ad>>, angle: f64ad) -> OptimaRotationAD {
        let data = Rotation3::from_axis_angle(axis, angle);
        return Self::new_rotation_matrix(data);
    }
    pub fn new_unit_quaternion_from_axis_angle(axis: &Unit<Vector3<f64ad>>, angle: f64ad) -> OptimaRotationAD {
        let data = UnitQuaternion::from_axis_angle(axis, angle);
        return Self::new_unit_quaternion(data);
    }
    pub fn new_rotation_matrix_from_lookat(lookat_direction: Vector3<f64ad>, lookat_axis: LookatAxis) -> OptimaRotationAD {
        assert!(lookat_direction.norm() > 0.0);
        ...
        let multiplier = match lookat_axis {
            LookatAxis::X => {f64ad::f64(1.0)}
            LookatAxis::Y => {f64ad::f64(1.0)}
            LookatAxis::Z => {f64ad::f64(1.0)}
            LookatAxis::NegX => {f64ad::f64(-1.0)}
            LookatAxis::NegY => {f64ad::f64(-1.0)}
            LookatAxis::NegZ => {f64ad::f64(-1.0)}
        };
        let column_a = (lookat_column + 1) % 3;
        let column_b = (lookat_column + 2) % 3;
        ...
        let mut u = Vector3::new(f64ad::f64(0.),f64ad::f64(0.),f64ad::f64(1.));
        let dot_test = l.dot(&u);
        if dot_test == 1.0 || dot_test == -1.0 {
            u = Vector3::new(f64ad::f64(0.),f64ad::f64(0.),f64ad::f64(1.));
        }
        ...
        let data = Rotation3::from_matrix_eps(&mat, f64ad::f64(0.0001), 1000, Rotation3::identity());
        let e = data.euler_angles();
        if e.0 == 0.0 && e.1 == 0.0 && e.2 == 0.0 {
            return Self::new_rotation_matrix_from_lookat(lookat_direction + Vector3::new(f64ad::f64(0.00001), f64ad::f64(0.00001), f64ad::f64(0.00001)), lookat_axis);
        }
        return Self::new_rotation_matrix(data);
    }
    pub fn new_quaternion_from_lookat(lookat_direction: Vector3<f64ad>, lookat_axis: LookatAxis) -> OptimaRotationAD {
        let r = Self::new_rotation_matrix_from_lookat(lookat_direction, lookat_axis);
        return r.convert(&OptimaRotationType::UnitQuaternion);
    }
    /// Creates new rotation by exponentating the logarithm vector (the vector returned by ln() function).
    pub fn new_from_exp(ln_vec: &Vector3<f64ad>, rotation_type: &OptimaRotationType) -> Self {
        return match rotation_type {
            OptimaRotationType::RotationMatrix => {
                let data = Rotation3::new(ln_vec.clone());
                Self::new_rotation_matrix(data)
    ...
    /// Converts the rotation to another provided rotation type.
    pub fn convert(&self, target_type: &OptimaRotationType) -> OptimaRotationAD {
        return match self {
            OptimaRotationAD::RotationMatrix { data, .. } => {
                match target_type {
                    OptimaRotationType::RotationMatrix => {
                        self.clone()
                    }
            ...
            OptimaRotationAD::UnitQuaternion { data, .. } => {
                match target_type {
                    OptimaRotationType::RotationMatrix => {
                        let data: Rotation3<f64ad> = data.to_rotation_matrix();
                        Self::new_rotation_matrix(data)
                    }
                    OptimaRotationType::UnitQuaternion => {
                        self.clone()
    ...
    /// Inverse rotation such that R * R^-1 = I
    pub fn inverse(&self) -> OptimaRotationAD {
        return match self {
            OptimaRotationAD::RotationMatrix { data, .. } => {
                let new_data = data.inverse();
                Self::new_rotation_matrix(new_data)
    ...
    /// The angle that is encoded by the given rotation
    pub fn angle(&self) -> f64ad {
        return match self {
            OptimaRotationAD::RotationMatrix { data, .. } => { data.angle() }
            OptimaRotationAD::UnitQuaternion { data, .. } => { data.angle() }
        }
    }
    /// Natural logarithm of the rotation.  This can be thought of as the rotation axis that is
    /// scaled by the length of the angle of rotation.
    pub fn ln(&self) -> Vector3<f64ad> {
        return match self {
            OptimaRotationAD::RotationMatrix { data, .. } => {
                let scaled_axis = data.scaled_axis();
                scaled_axis
            ...
            OptimaRotationAD::UnitQuaternion { data, .. } => {
                let out_vec: Vector3<f64ad> = data.ln().vector().into();
                out_vec
            }
        }
    }
    /// Rotation multiplication.
    pub fn multiply(&self, other: &OptimaRotationAD, conversion_if_necessary: bool) -> Result<OptimaRotationAD, OptimaError> {
        if self.get_rotation_type() != other.get_rotation_type() {
            return if conversion_if_necessary {
                let new_operand = other.convert(self.get_rotation_type());
                self.multiply(&new_operand, conversion_if_necessary)
        ...
        return match self {
            ...
            OptimaRotationAD::UnitQuaternion { data, rotation_type: _ } => {
                let data0 = data;
                match other {
                    OptimaRotationAD::UnitQuaternion { data, .. } => {
                        let new_data = data0 * data;
    ...
    /// The displacement between two rotations such that R_self * R_displacement = R_other
    pub fn displacement(&self, other: &OptimaRotationAD, conversion_if_necessary: bool) -> Result<OptimaRotationAD, OptimaError> {
        if self.get_rotation_type() != other.get_rotation_type() {
            return if conversion_if_necessary {
                let new_operand = other.convert(self.get_rotation_type());
                self.displacement(&new_operand, conversion_if_necessary)
    ...
    /// The angle between two rotations.
    pub fn angle_between(&self, other: &OptimaRotationAD, conversion_if_necessary: bool) -> Result<f64ad, OptimaError> {
        if self.get_rotation_type() != other.get_rotation_type() {
            return if conversion_if_necessary {
                let new_operand = other.convert(self.get_rotation_type());
                self.angle_between(&new_operand, conversion_if_necessary)
    ...
    /// Returns the 3x3 rotation matrix encoded by the rotation object.  Returns error if the
    /// underlying representation is not a RotationMatrix.
    pub fn unwrap_rotation_matrix(&self) -> Result<&Rotation3<f64ad>, OptimaError> {
        return match self {
            OptimaRotationAD::RotationMatrix { data, .. } => {
                Ok(data)
            }
    ...
    /// Returns the Unit Quaternion encoded by the rotation object.  Returns error if the
    /// underlying representation is not a UnitQuaternion.
    pub fn unwrap_unit_quaternion(&self) -> Result<&UnitQuaternion<f64ad>, OptimaError> {
        return match self {
    ...
    /// Returns the euler angle representation of the rotation.
    pub fn to_euler_angles(&self) -> Vector3<f64ad> {
        let euler_angles = match self {
            OptimaRotationAD::RotationMatrix { data, .. } => { data.euler_angles() }
            OptimaRotationAD::UnitQuaternion { data, .. } => { data.euler_angles() }
        };
        let euler_angles_vec = Vector3::new(euler_angles.0, euler_angles.1, euler_angles.2);
        return euler_angles_vec;
    }
    /// To axis angle representation of a rotation.
    pub fn to_axis_angle(&self) -> (Vector3<f64ad>, f64ad) {
        let axis_angle = match self {
        ...
        match axis_angle {
            None => {
                (Vector3::new(f64ad::f64(0.),f64ad::f64(0.),f64ad::f64(0.)), f64ad::f64(0.))
            }
            Some(axis_angle) => {
                (Vector3::new(axis_angle.0[0], axis_angle.0[1], axis_angle.0[2]), axis_angle.1)
            }
    ...
    /// Spherical linear interpolation.
    pub fn slerp(&self, other: &OptimaRotationAD, t: f64ad, conversion_if_necessary: bool) -> Result<OptimaRotationAD, OptimaError> {
        if self.get_rotation_type() != other.get_rotation_type() {
            return if conversion_if_necessary {
                let new_operand = other.convert(self.get_rotation_type());
                self.slerp(&new_operand, t, conversion_if_necessary)
    ...
    fn get_rotation_type(&self) -> &OptimaRotationType {
        return match &self {
            OptimaRotationAD::RotationMatrix { data: _, rotation_type } => { rotation_type }
            OptimaRotationAD::UnitQuaternion { data: _, rotation_type } => { rotation_type }
        }
    }
    /// Converts to vector representation.
    ///
