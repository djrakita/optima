use nalgebra::{Matrix3, Matrix4, Rotation, Rotation3, UnitQuaternion, Vector3};
...
impl HomogeneousMatrix {
    ...
    pub fn inverse(&self) -> Self {
        ...
        matrix[(0, 1)] = self.matrix[(1, 0)];
        matrix[(0, 2)] = self.matrix[(2, 0)];
        matrix[(1, 0)] = self.matrix[(0, 1)];
        matrix[(1, 2)] = self.matrix[(1, 2)];
        matrix[(2, 0)] = self.matrix[(0, 2)];
        matrix[(2, 1)] = self.matrix[(1, 2)];

        matrix[(0, 3)] = -self.matrix[(0, 3)];
        matrix[(1, 3)] = -self.matrix[(1, 3)];
        matrix[(2, 3)] = -self.matrix[(2, 3)];
