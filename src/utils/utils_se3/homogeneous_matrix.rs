...
/// A representation for an SE(3) transform composed of a 4x4 homogeneous transformation matrix.
#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct HomogeneousMatrixAD {
    matrix: Matrix4<f64ad>
}
impl HomogeneousMatrixAD {
    pub fn new(matrix: Matrix4<f64ad>) -> Self {
        Self {
            matrix
        }
    }
    pub fn new_from_euler_angles(rx: f64ad, ry: f64ad, rz: f64ad, x: f64ad, y: f64ad, z: f64ad) -> Self {
        let rotation = OptimaRotationAD::new_rotation_matrix_from_euler_angles(rx, ry, rz);
        let translation = Vector3::new(x, y, z);
        let matrix = Self::rotation_and_translation_to_homogeneous_matrix(&rotation, &translation);
        return Self::new(matrix);
    }
    pub fn new_from_axis_angle(axis: &Unit<Vector3<f64ad>>, angle: f64ad, x: f64ad, y: f64ad, z: f64ad) -> Self {
        let rotation = OptimaRotationAD::new_rotation_matrix_from_axis_angle(axis, angle);
        let translation = Vector3::new(x, y, z);
        let matrix = Self::rotation_and_translation_to_homogeneous_matrix(&rotation, &translation);
        return Self::new(matrix);
    }
    /// Returns the rotation component of the homogeneous matrix.
    pub fn rotation(&self) -> Rotation3<f64ad> {
        let mut mat3 = Matrix3::zeros();
    ...
    /// multiplication
    pub fn multiply(&self, other: &HomogeneousMatrixAD) -> HomogeneousMatrixAD {
        let matrix = self.matrix * &other.matrix;
        return Self::new(matrix);
    }
    /// multiplication by a point
    pub fn multiply_by_point(&self, point: &Vector3<f64ad>) -> Vector3<f64ad> {
        let four_point = Vector4::new(point[0], point[1], point[2], f64ad::f64(1.0));
        let result_point = self.matrix * &four_point;
        return Vector3::new(result_point[0], result_point[1], result_point[2]);
    }
    /// Inverse multiplies by the given point.  inverse multiplication is useful for placing the
    ...
    pub fn inverse(&self) -> Self {
        ...
        matrix[(3,3)] = f64ad::f64(1.0);

        return Self::new(matrix);
    }
    /// The displacement transform such that T_self * T_disp = T_other.
    pub fn displacement(&self, other: &HomogeneousMatrixAD) -> HomogeneousMatrixAD {
        return self.inverse().multiply(&other);
    }
    pub fn displacement_separate_rotation_and_translation(&self, other: &HomogeneousMatrixAD) -> (OptimaRotationAD, Vector3<f64ad>) {
        let disp_rotation = OptimaRotationAD::new_rotation_matrix(self.rotation().clone()).displacement(&OptimaRotationAD::new_rotation_matrix(other.rotation().clone()), false).expect("error");
        let disp_translation = other.translation() - self.translation();
        return (disp_rotation, disp_translation);
    }
    pub fn slerp(&self, other: &HomogeneousMatrixAD, t: f64ad) -> HomogeneousMatrixAD {
        let new_rot = OptimaRotationAD::new_rotation_matrix(self.rotation().slerp(&other.rotation(), t));
        let new_translation = (1.0 - t) * self.translation() + t * other.translation();
        let matrix = Self::rotation_and_translation_to_homogeneous_matrix(&new_rot, &new_translation);
        return Self::new(matrix);
    }
    /// Provides an approximate distance between two homogeneous matrices.  This is not an
    /// official distance metric, but should still work in some optimization procedures.
    pub fn approximate_distance(&self, other: &HomogeneousMatrixAD) -> f64ad {
        let angle_between = self.rotation().angle_to(&other.rotation());
        let translation_between = (self.translation() - other.translation()).norm();
        return angle_between + translation_between;
    }
    /// Convenience function for mapping rotation and translation components to a 4x4 matrix.
    pub fn rotation_and_translation_to_homogeneous_matrix(rotation: &OptimaRotationAD, translation: &Vector3<f64ad>) -> Matrix4<f64ad> {
        let rot_mat = rotation.convert(&OptimaRotationType::RotationMatrix)
            .unwrap_rotation_matrix()
            .expect("error")
            .clone();
    ...
    pub fn convert(&self, target_type: &OptimaSE3PoseType) -> OptimaSE3PoseAD {
        return match target_type {
            OptimaSE3PoseType::ImplicitDualQuaternion => {
                let rotation = UnitQuaternion::from_rotation_matrix(&self.rotation());
                let translation = self.translation();
                let data = ImplicitDualQuaternionAD::new(rotation, translation);
                OptimaSE3PoseAD::new_implicit_dual_quaternion(data)
            }
            OptimaSE3PoseType::HomogeneousMatrix => {
                OptimaSE3PoseAD::new_homogeneous_matrix(self.clone())
            }
            OptimaSE3PoseType::UnitQuaternionAndTranslation => {
                let rotation = OptimaRotationAD::new_unit_quaternion(UnitQuaternion::from_rotation_matrix(&self.rotation()));
                let translation = self.translation();
                let data = RotationAndTranslationAD::new(rotation, translation);
                OptimaSE3PoseAD::new_rotation_and_translation(data)
            }
            OptimaSE3PoseType::RotationMatrixAndTranslation => {
                let rotation_matrix = self.rotation();
                let translation = self.translation();
                let data = RotationAndTranslationAD::new(OptimaRotationAD::new_rotation_matrix(rotation_matrix), translation);
                OptimaSE3PoseAD::new_rotation_and_translation(data)
            }
            OptimaSE3PoseType::EulerAnglesAndTranslation => {
                let euler_angles_and_rotation = self.to_euler_angles_and_translation();
